...
class FastAPILLM implements LLMAdapter {
  async generateResponse(
    ...
    const webSearchError = data.web_search_error;
    const attemptedWeb = usedWebFallback || Boolean(webSearchError);
    ...
    const toolResults: ToolResult[] = attemptedWeb ? [{
      toolName: 'web_search',
      query: lastUserMessage.content,
      result: { results: webSearchResults, ...(webSearchError ? { error: webSearchError } : {}) },
      timestamp: new Date(),
      type: 'web_search',
    }] : [];
    ...
    if (webSearchError && !usedWebFallback) {
      confidence = 'low';
    } else if (topSimilarity > 0.85) {
      // High similarity - reliable even with web augmentation
      confidence = 'high';
    } else if (topSimilarity > 0.7) {
      // Medium similarity
      confidence = 'medium';
    } else if (source === 'web') {
      // Pure web search without good document match
      confidence = 'low';
    } else if (source === 'hybrid' && topSimilarity > 0.6) {
      // Hybrid with decent similarity
      confidence = 'medium';
    } else {
      // Low similarity or no documents
      confidence = 'low';
    }
