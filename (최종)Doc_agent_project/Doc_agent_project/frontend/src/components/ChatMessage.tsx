...
import { ChatMessage as ChatMessageType, ToolResult, WebSearchResultItem } from '@/types';
...
export function ChatMessage({ message }: ChatMessageProps) {
  ...
  const renderWebSearchResults = (results: WebSearchResultItem[]) => (
    <div className="space-y-2">
      {results.map((result, idx) => (
        <div key={result.url || idx.toString()} className="text-xs">
          <a
            href={result.url}
            target="_blank"
  ...
  const renderToolResultContent = (tool: ToolResult) => {
    switch (tool.type) {
      case 'web_search':
        if ('error' in tool.result && tool.result.error) {
          return (
            <p className="text-xs text-destructive">
              웹 검색 오류: {tool.result.error}
            </p>
          );
        }
        return renderWebSearchResults(tool.result.results ?? []);
      case 'calculator':
        return (
          <p className="text-xs text-muted-foreground">
            {'error' in tool.result && tool.result.error
              ? `오류: ${tool.result.error}`
              : `결과: ${tool.result.value ?? 'N/A'}`}
          </p>
        );
      case 'fetch_url':
        return (
          <div className="text-xs space-y-1">
            {tool.result.title && <p className="font-medium">{tool.result.title}</p>}
            {tool.result.content && <p className="text-muted-foreground">{tool.result.content}</p>}
          </div>
        );
      default:
        return (
          <pre className="text-[11px] bg-muted/40 p-2 rounded">
