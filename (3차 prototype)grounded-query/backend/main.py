...
RAG_SYSTEM_MSG = (
    ...
    "- 문서에 정보가 없을 때만 http_fetch tool을 사용해 Serper API로 검색할 수 있다.\n"
    "- 중요: 절대 직접 웹사이트 URL을 호출하지 말고, 반드시 Serper API만 사용하라.\n"
    "- Serper API 호출 방법:\n"
...
async def ask_question(payload: AskPayload) -> AskResponse:
    ...
    require_web_search = not context_chunks or (not doc_confident)
        ...
        if use_web_search:
            ...
            messages.append({
                ...
                "content": (
                    f"[문서 컨텍스트]\n{context_text}\n\n"
                    f"[질문]\n{payload.question}\n\n"
                    "위 문서 내용을 먼저 요약하고 활용하여 초안을 만든 뒤, 반드시 http_fetch tool을 사용해 Serper API로 추가 정보를 검색하고, "
            ...
            messages.append({
                ...
                "content": (
                    ...
                    "만약 문서에 관련 정보가 부족하다고 판단되면, http_fetch tool을 사용해서 Serper API로 웹 검색을 수행하세요."
                )
            })
    else:
    ...
    if doc_summary_text:
        ...
        messages.append({
            "role": "user",
            "content": "위 문서 요약을 최우선으로 활용해 답변하세요. 추가 정보가 명확히 필요할 때만 http_fetch tool을 호출하세요."
        })
